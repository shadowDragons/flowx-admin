...
import { roleCreate, roleDelete, roleFindAll, roleUpdate } from '@/services/flowx-api/role';
...
const [currentRow, setCurrentRow] = useState<APIV2.RoleListItem>();
const [selectedRows, setSelectedRows] = useState<APIV2.RoleListItem[]>([]);
 ...
 * @param fields
 */
const handleAdd = async (fields: APIV2.RoleListItem) => {
...
* @param fields
*/
const handleUpdate = async (fields: APIV2.RoleListItem) => {
  const hide = message.loading(intl.formatMessage({
    id: 'pages.table.op.ing',
    defaultMessage: 'pages.table.op.ing',
  }));
...
const handleRemove = async (ids: number[]) => {
  ...
  const columns: ProColumns<APIV2.RoleListItem>[] = [
    {
      title: <FormattedMessage id="pages.roleTable.title" defaultMessage="Description" />,
      dataIndex: 'name',
      key: 'name'
    },
    {
  ...
  return (
    <PageContainer>
      <ProTable<APIV2.RoleListItem, APIV2.RoleFindAllParams>
        actionRef={actionRef}
        headerTitle={intl.formatMessage({
          id: 'pages.roleTable.title',
          defaultMessage: 'pages.roleTable.title',
        })}
        search={{
          labelWidth: 120,
        }}
        ...
        request={async (params) => {
          return await roleFindAll(params);
        }}
        columns={columns}
        rowSelection={{
    ...
    >
        <ProFormGroup title={intl.formatMessage({
            id: 'pages.roleTable.name',
            defaultMessage: 'pages.roleTable.name',
        })}>
            <ProFormText width="md" name="name" />
        </ProFormGroup>
    </ModalForm>
    <UpdateForm
        onSubmit={async (value : APIV2.RoleListItem) => {
